 ...
 *
 * Just like acc::detail::uintToBuffer in Conv.h, writes at the *end* of
...
void BaseFormatter<Derived, containerMode, Args...>::operator()(
  ...
  auto outputString = [&out](StringPiece s) {
    ...
    while (p != end) {
      ...
      if (p == end || *p != '}') {
        throwBadFormatArg("acc::format: single '}' in format string");
      }
      ++p;
    }
  };
  ...
  while (p != end) {
    ...
    if (p == end) {
      throwBadFormatArg("acc::format: '}' at end of format string");
    }
    ...
    if (q == nullptr) {
      throwBadFormatArg("acc::format: missing ending '}'");
    }
    ...
    if (hasDefaultArgIndex && hasExplicitArgIndex) {
      throwBadFormatArg(
          "acc::format: may not have both default and explicit arg indexes");
    }
...
void formatString(StringPiece val, FormatArg& arg, FormatCallback& cb) {
  if (arg.width != FormatArg::kDefaultWidth && arg.width < 0) {
    throwBadFormatArg("acc::format: invalid width");
  }
  if (arg.precision != FormatArg::kDefaultPrecision && arg.precision < 0) {
    throwBadFormatArg("acc::format: invalid precision");
  }
 ...
 * compatible with acc::toAppend and acc::to.
 */
