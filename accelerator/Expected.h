 ...
 */
/**
 * Like acc::Optional, but can store a value *or* an error.
 *
...
#include "accelerator/ColdClass.h"
...
#include "accelerator/Unit.h"
#include "accelerator/Utility.h"

#define ACC_EXPECTED_ID(X) ACC_CONCATENATE(ACC_CONCATENATE(acc, X), __LINE__)

#define ACC_REQUIRES_IMPL(...)                                            \
  bool ACC_EXPECTED_ID(Requires) = false,                                 \
       typename std::enable_if<                                           \
           (ACC_EXPECTED_ID(Requires) || static_cast<bool>(__VA_ARGS__)), \
           int>::type = 0

#define ACC_REQUIRES_TRAILING(...) , ACC_REQUIRES_IMPL(__VA_ARGS__)

#define ACC_REQUIRES(...) template <ACC_REQUIRES_IMPL(__VA_ARGS__)>
...
struct ExpectedHelper {
  ...
  static constexpr Expected<T, Error> return_(T t) {
    return acc::makeExpected<Error>(t);
  }
  template <
      class Error,
      class T,
      class U ACC_REQUIRES_TRAILING(
  ...
  static auto then_(This&& ex, Fn&& fn, Fns&&... fns) -> decltype(T::then_(
      ...
      std::declval<Fns>()...)) {
    if (ACC_LIKELY(ex.which_ == expected_detail::Which::eValue)) {
      return T::then_(
          T::template return_<E>(
  ...
  template <
      ...
      class Err = decltype(std::declval<No>()(std::declval<This>().error()))
          ACC_REQUIRES_TRAILING(!std::is_void<Err>::value)>
  ...
  static Ret thenOrThrow_(This&& ex, Yes&& yes, No&& no) {
    ...
    throw bad(static_cast<This&&>(ex).error());
  }
};
} // namespace expected_detail_ExpectedHelper
 ...
 */
class ACC_EXPORT BadExpectedAccess : public std::logic_error {
 public:
...
class Unexpected final : ColdClass {
 ...
 public:
   ...
   */
  class ACC_EXPORT BadExpectedAccess : public acc::BadExpectedAccess {
   public:
    explicit BadExpectedAccess(Error err)
        : acc::BadExpectedAccess{}, error_(std::move(err)) {}
    /**
  ...
  template <class Other ACC_REQUIRES_TRAILING(
...
template <
    class Error ACC_REQUIRES_TRAILING(IsEqualityComparable<Error>::value)>
 ...
 * or error object causes Expected to throw acc::BadExpectedAccess.
 *
...
class Expected final : expected_detail::ExpectedStorage<Value, Error> {
 ...
 public:
  ...
  template <class... Ts ACC_REQUIRES_TRAILING(
  ...
  template <
      class U,
      class... Ts ACC_REQUIRES_TRAILING(
  ...
  ACC_REQUIRES(std::is_copy_constructible<Error>::value)
  constexpr Expected(unexpected_t, const Error& err) noexcept(
      noexcept(Error(err)))
      : Base{expected_detail::ErrorTag{}, err} {}

  ACC_REQUIRES(std::is_move_constructible<Error>::value)
  ...
  ACC_REQUIRES(expected_detail::IsCopyable<Value>::value)
  ...
  ACC_REQUIRES(expected_detail::IsMovable<Value>::value)
  ...
  ACC_REQUIRES(expected_detail::IsCopyable<Error>::value)
  ...
  ACC_REQUIRES(expected_detail::IsMovable<Error>::value)
  ...
  constexpr bool hasValue() const noexcept {
    return ACC_LIKELY(expected_detail::Which::eValue == this->which_);
  }

  constexpr bool hasError() const noexcept {
    return ACC_UNLIKELY(expected_detail::Which::eError == this->which_);
  }
  ...
  Value value_or(U&& dflt) const& {
    if (ACC_LIKELY(this->which_ == expected_detail::Which::eValue)) {
      return this->value_;
    }
   ...
   * then
   */
  template <class... Fns ACC_REQUIRES_TRAILING(sizeof...(Fns) >= 1)>
 ...
 private:
  void requireValue() const {
    if (ACC_UNLIKELY(!hasValue())) {
      if (ACC_LIKELY(hasError())) {
        using Err = typename Unexpected<Error>::BadExpectedAccess;
        throw Err(this->error_);
      }
      throw BadExpectedAccess();
    }
  }
...
operator==(
  ...
  if (ACC_UNLIKELY(lhs.uninitializedByException())) {
    throw BadExpectedAccess();
  }
  if (ACC_UNLIKELY(lhs.which_ != rhs.which_)) {
    return false;
  }
  if (ACC_UNLIKELY(lhs.hasError())) {
...
template <
    class Value,
    class Error ACC_REQUIRES_TRAILING(IsEqualityComparable<Value>::value)>
inline bool operator!=(
    const Expected<Value, Error>& lhs,
    const Expected<Value, Error>& rhs) {
  return !(rhs == lhs);
...
template <
    class Value,
    class Error ACC_REQUIRES_TRAILING(IsLessThanComparable<Value>::value)>
inline bool operator<=(
    const Expected<Value, Error>& lhs,
    const Expected<Value, Error>& rhs) {
  return !(rhs < lhs);
