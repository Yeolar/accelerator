...
bool splitFixed(const Delim& delimiter, StringPiece input, OutputType& output) {
  ...
  if (exact && ACC_UNLIKELY(std::string::npos != input.find(delimiter))) {
    return false;
  }
  output = acc::to<OutputType>(input);
  return true;
}
...
bool splitFixed(
  ...
  if (ACC_UNLIKELY(cut == std::string::npos)) {
    return false;
  }
  StringPiece head(input.begin(), input.begin() + cut);
  StringPiece tail(input.begin() + cut + detail::delimSize(delimiter),
                   input.end());
  if (ACC_LIKELY(splitFixed<exact>(delimiter, tail, outTail...))) {
    outHead = acc::to<OutputType>(head);
    return true;
  }
  return false;
}
...
void toAppendPair(Delim delimiter, Pair pair, std::string& output) {
  toAppend(pair.first, delimiter, pair.second, &output);
}

template <class Delim, class Iterator>
void internalMapJoinAppend(Delim delimiter,
                           Delim pairDelimiter,
                           Iterator begin,
                           Iterator end,
                           std::string& output) {
  ...
  while (++begin != end) {
    toAppend(delimiter, pairDelimiter, *begin, &output);
  }
}
