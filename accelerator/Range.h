...
class Range {
 ...
 public:
  ...
  size_type rfind(value_type c) const {
    return acc::rfind(castToConst(), c);
  }
   ...
   * Example:
   *
   *  acc::StringPiece s("sample string for split_next");
   *  auto p = s.split_step(' ');
   *
   ...
   * Example:
   *
   *  void do_some_parsing(acc::StringPiece s) {
   *    auto version = s.split_step(' ', [&](acc::StringPiece x) {
   *      if (x.empty()) {
   ...
   *
   *  struct Foo {
   *    void parse(acc::StringPiece s) {
   *      s.split_step(' ', parse_field, bar, 10);
   *      s.split_step('\t', parse_field, baz, 20);
   *
   *      auto const kludge = [](acc::StringPiece x, int &out, int def) {
   *        if (x == "null") {
   *          out = 0;
   *        } else {
   ...
   *    int foo;
   *
   *    static parse_field(acc::StringPiece s, int &out, int def) {
   *      try {
   *        out = acc::to<int>(s);
