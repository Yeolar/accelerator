...
#include "accelerator/gen/Core.h"
...
class To {
 public:
  template <class Value>
  Dest operator()(Value&& value) const {
    return ::acc::to<Dest>(std::forward<Value>(value));
  }
};
...
#define GENERATOR(TYPE)                            \
  ::acc::gen::detail::GeneratorBuilder<TYPE>() + \
...
#include "accelerator/gen/Base-inl.h"
