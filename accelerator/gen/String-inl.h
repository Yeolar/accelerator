...
#ifndef ACC_GEN_STRING_H
#error This file may only be included from accelerator/gen/String.h
#endif
...
bool StreamSplitter<Callback>::flush() {
  ACCCHECK(maxLength_ == 0 || buffer_.length() < maxLength_);
...
bool StreamSplitter<Callback>::operator()(StringPiece in) {
  ...
  if (buffer_.length() != 0) {
    if (found) {
      ...
      if (maxLength_) {
        ACCCHECK_LT(buffer_.length(), maxLength_);
