...
TEST(StringGen, EachToTuple) {
  ...
  }
  {
    ...
    auto actual
      = split(lines, ' ')
      | eachToTuple<int, std::string, int>(':')
      | as<vector>();
    vector<tuple<int, std::string, int>> expected {
...
void checkResplitMaxLength(vector<string> ins,
  ...
  if (ins.size() > 1) {
    checkResplitMaxLength({acc::join("", ins)}, delim, maxLength, outs);
  }
}
...
TEST(StringGen, Unsplit) {
  ...
  auto existingBuffer = [](StringPiece s) {
    ...
    auto expected = acc::to<std::string>(
        "asdf", s.empty() ? "" : ",", s);
