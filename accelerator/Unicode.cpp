...
#include "accelerator/Utf8StringPiece.h"
...
char32_t utf8ToCodePoint(const unsigned char*& p,
  ...
  if ((fst & 0xC0) != 0xC0) {
    ...
    throw std::runtime_error(
      to<std::string>("acc::utf8ToCodePoint i=0 d=", d));
  }
  ...
  for (unsigned int i = 1; i != 3 && p + i < e; ++i) {
    ...
    if ((tmp & 0xC0) != 0x80) {
      if (skipOnError) return skip();
      throw std::runtime_error(
        to<std::string>("acc::utf8ToCodePoint i=", i,
                        " tmp=", (uint32_t)tmp));
    }
...
std::string toLower(const acc::StringPiece& sp) {
  std::string result;
...
std::string toUpper(const acc::StringPiece& sp) {
  std::string result;
  icu::StringPiece usp(sp.begin(), (int32_t)sp.size());
...
std::string toOppositeCase(const acc::StringPiece& sp) {
  std::string result;
...
std::string normalize(const acc::StringPiece& sp, NormalizationMode mode) {
  std::string result;
  icu::StringPiece usp(sp.begin(), (int32_t)sp.size());
  ...
  if (err != U_ZERO_ERROR) {
    ACCLOG(WARN) << "normalize error: " << err;
  }
  out.toUTF8String(result);
  return result;
}

std::string asciify(const acc::StringPiece& sp) {
  std::string result;
  icu::StringPiece usp(sp.begin(), (int32_t)sp.size());
  icu::UnicodeString us = icu::UnicodeString::fromUTF8(usp);
  UErrorCode err = U_ZERO_ERROR;
  ...
  if (err != U_ZERO_ERROR) {
    ACCLOG(WARN) << "Create Transliterator instance error: " << err;
  }
