 ...
 * ::acc::to() will call makeConversionError to translate that error code into
 ...
 *
 * YourConversionError
 * makeConversionError(YourErrorCode code, ::acc::StringPiece sp) {
 *   ...
 ...
 * acc::to<SomeString>(double), generic implementation above will
 ...
 */
template <typename Tgt>
ACC_NODISCARD inline typename std::enable_if<
...
convertTo(const Src& value) noexcept {
  /* static */ if (
      acc::_t<std::make_unsigned<Tgt>>(std::numeric_limits<Tgt>::max()) <
      acc::_t<std::make_unsigned<Src>>(std::numeric_limits<Src>::max())) {
...
typename std::enable_if<
    ...
    Expected<Tgt, ConversionCode>>::type
convertTo(const Src& value) noexcept {
  if (ACC_LIKELY(checkConversion<Tgt>(value))) {
    Tgt result = static_cast<Tgt>(value);
    if (ACC_LIKELY(checkConversion<Src>(result))) {
      Src witness = static_cast<Src>(result);
      if (ACC_LIKELY(value == witness)) {
        return result;
      }
    }
  }
 ...
 * Custom Conversions
 *
 * Any type can be used with acc::to by implementing parseTo. The
 * implementation should be provided in the namespace of the type to facilitate
 * argument-dependent lookup:
 *
 * namespace other_namespace {
 * ::acc::Expected<::acc::StringPiece, SomeErrorCode>
 *   parseTo(::acc::StringPiece, OtherType&) noexcept;
 * }
 ******************************************************************************/
template <class T>
ACC_NODISCARD typename std::enable_if<
    std::is_enum<T>::value,
    Expected<StringPiece, ConversionCode>>::type
...
struct CheckTrailingSpace {
  Expected<Unit, ConversionCode> operator()(StringPiece sp) const {
    ...
    if (ACC_UNLIKELY(e != ConversionCode::SUCCESS)) {
