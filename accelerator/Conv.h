...
#define ACC_RANGE_CHECK(condition, message, src)              \
  ((condition) ? (void)0 : throw std::range_error(            \
    (std::string(__FILE__ "(" ACC_STRINGIZE2(__LINE__) "): ") \
     + (message) + ": '" + (src) + "'").c_str()))

#define ACC_RANGE_CHECK_BEGIN_END(condition, message, b, e)   \
  ACC_RANGE_CHECK(condition, message, std::string((b), (e) - (b)))

#define ACC_RANGE_CHECK_STRINGPIECE(condition, message, sp)   \
  ACC_RANGE_CHECK(condition, message, std::string((sp).data(), (sp).size()))
...
to(const Src & value) {
  /* static */ if (std::numeric_limits<Tgt>::max()
    ...
    ACC_RANGE_CHECK(value <= std::numeric_limits<Tgt>::max(),
      "Overflow",
      std::to_string(value));
  }
  /* static */ if (std::is_signed<Src>::value &&
                   (!std::is_signed<Tgt>::value || sizeof(Src) > sizeof(Tgt))) {
    ACC_RANGE_CHECK(value >= std::numeric_limits<Tgt>::min(),
      "Negative overflow",
      std::to_string(value));
  }
...
inline uint32_t digits10(uint64_t v) {
  ...
  static const uint64_t powersOf10[20] ACC_ALIGNED(64) = {
...
estimateSpaceNeeded(Src value) {
  const char *c = value;
  if (c) {
    return acc::StringPiece(value).size();
  };
  return 0;
}
...
template<class Src>
typename std::enable_if<
  (std::is_convertible<Src, acc::StringPiece>::value ||
...
ACC_ALIGNED(16) const uint16_t shift1[] = {
...
ACC_ALIGNED(16) const uint16_t shift10[] = {
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  // 0-9
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  10
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  20
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  30
...
ACC_ALIGNED(16) const uint16_t shift100[] = {
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  // 0-9
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  10
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  20
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  30
...
ACC_ALIGNED(16) const uint16_t shift1000[] = {
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  // 0-9
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  10
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  20
  OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR, OOR,  //  30
 ...
 */
  ...
  Tgt digits_to(const char * b, const char * e) {
    ...
    assert(b == e);
    ACC_RANGE_CHECK_BEGIN_END(size > 0,
...
typename std::enable_if<
  ...
  Tgt>::type
to(const char * b, const char * e) {
  ACC_RANGE_CHECK(b < e, "Empty input string in conversion to integral",
                  to<std::string>("b: ", intptr_t(b), " e: ", intptr_t(e)));
  if (!isdigit(*b)) {
    if (*b == '-') {
      Tgt result = -to<typename std::make_unsigned<Tgt>::type>(b + 1, e);
      ACC_RANGE_CHECK_BEGIN_END(result <= 0, "Negative overflow.", b, e);
      return result;
    }
    ACC_RANGE_CHECK_BEGIN_END(*b == '+', "Invalid lead character", b, e);
    ++b;
  }
  Tgt result = to<typename std::make_unsigned<Tgt>::type>(b, e);
  ACC_RANGE_CHECK_BEGIN_END(result >= 0, "Overflow", b, e);
  return result;
}
...
to(StringPiece * src) {
  ...
  for (;; ++b) {
    ACC_RANGE_CHECK_STRINGPIECE(b < past,
  ...
  /* static */ if (std::is_signed<Tgt>::value) {
    if (!isdigit(*m)) {
      if (*m == '-') {
        ...
        ACC_RANGE_CHECK_STRINGPIECE(*m == '+', "Invalid leading character in "
  ...
  ACC_RANGE_CHECK_STRINGPIECE(m < past, "No digits found in input string",
                              *src);
  ACC_RANGE_CHECK_STRINGPIECE(isdigit(*m), "Non-digit character found", *src);
  ...
  /* static */ if (!std::is_signed<Tgt>::value) {
    ...
    if (negative) {
      result = -t;
      ACC_RANGE_CHECK_STRINGPIECE(result <= 0, "Negative overflow", *src);
    } else {
      result = t;
      ACC_RANGE_CHECK_STRINGPIECE(result >= 0, "Overflow", *src);
    }
  }
...
inline void enforceWhitespace(const char* b, const char* e) {
  for (; b != e; ++b) {
    ACC_RANGE_CHECK_BEGIN_END(isspace(*b),
...
to(StringPiece *const src) {
  ...
  ACC_RANGE_CHECK_STRINGPIECE(!src->empty(),
                              "No digits found in input string", *src);
...
// ACC_CONV_INTERNAL is defined by Conv.cpp.  Keep the ACC_RANGE_CHECK
...
#ifndef ACC_CONV_INTERNAL
#undef ACC_RANGE_CHECK
#undef ACC_RANGE_CHECK_BEGIN_END
#undef ACC_RANGE_CHECK_STRINGPIECE
#endif
