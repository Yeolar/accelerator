...
/**
 * This functions is an extension point when ACC_HAVE_WEAK_SYMBOLS is true.
 ...
 *                    type. It may be an empty std::function.
 */
std::function<void(double)> makeCompressionCounterHandler(
    acc::io::CodecType codecType,
    acc::StringPiece codecName,
    acc::Optional<int> level,
...
class CompressionCounter {
 ...
 private:
  ...
  std::function<std::function<void(double)>()> initialize_;
  std::function<void(double)> increment_;
};
