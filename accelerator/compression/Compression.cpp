...
#if ACC_HAVE_LIBSNAPPY
#include <snappy-sinksource.h>
#include <snappy.h>
#endif

#if ACC_HAVE_LIBZ
#include "accelerator/compression/Zlib.h"
#endif
...
#if ACC_HAVE_LIBZSTD
#include "accelerator/compression/Zstd.h"
#endif
...
#include "accelerator/TimeWatcher.h"
#include "accelerator/Varint.h"
#include "accelerator/compression/Utils.h"
#include "accelerator/io/Cursor.h"
#include <algorithm>
#include <unordered_set>

using acc::io::compression::detail::dataStartsWithLE;
using acc::io::compression::detail::prefixToStringLE;
...
class Timer {
 public:
  explicit Timer(acc::detail::CompressionCounter& counter)
      : counter_(&counter) {}

  ~Timer() {
    *counter_ += timer_.elapsed() / 1000000;
  }

 private:
  acc::detail::CompressionCounter* counter_;
  TimeWatcher timer_;
};
} // namespace
...
std::unique_ptr<IOBuf> Codec::compress(const IOBuf* data) {
  ...
  bool const logging = acc::Random::oneIn(kLoggingRate);
  acc::Optional<Timer> const timer =
      logging ? Timer(compressionMilliseconds_) : acc::Optional<Timer>();
...
std::unique_ptr<IOBuf> Codec::uncompress(
  ...
  bool const logging = acc::Random::oneIn(kLoggingRate);
  acc::Optional<Timer> const timer =
      logging ? Timer(decompressionMilliseconds_) : acc::Optional<Timer>();
...
void StreamCodec::assertStateIs(State expected) const {
  if (state_ != expected) {
    throw std::logic_error(acc::to<std::string>(
...
#if (ACC_HAVE_LIBLZ4 || ACC_HAVE_LIBLZMA)

namespace {

void encodeVarintToIOBuf(uint64_t val, acc::IOBuf* out) {
  DCHECK_GE(out->tailroom(), kMaxVarintLength64);
  out->append(encodeVarint(val, out->writableTail()));
}

inline uint64_t decodeVarintFromCursor(acc::io::Cursor& cursor) {
  uint64_t val = 0;
  int8_t b = 0;
...
#endif  // ACC_HAVE_LIBLZ4 || ACC_HAVE_LIBLZMA

#if ACC_HAVE_LIBLZ4

#if LZ4_VERSION_NUMBER >= 10802 && defined(LZ4_STATIC_LINKING_ONLY) && \
    defined(LZ4_HC_STATIC_LINKING_ONLY) && !defined(ACC_USE_LZ4_FAST_RESET)
#define ACC_USE_LZ4_FAST_RESET
#endif

#ifdef ACC_USE_LZ4_FAST_RESET
namespace {
...
class LZ4Codec final : public Codec {
 ...
 private:
  ...
  std::unique_ptr<
      LZ4_stream_t,
      acc::static_function_deleter<LZ4_stream_t, lz4_stream_t_deleter>>
      ctx;
  std::unique_ptr<
      LZ4_streamHC_t,
      acc::static_function_deleter<LZ4_streamHC_t, lz4_streamhc_t_deleter>>
      hcctx;
#endif
...
void LZMA2StreamCodec::resetCStream() {
  ...
  if (rc != LZMA_OK) {
    throw std::runtime_error(acc::to<std::string>(
...
AutomaticCodec::AutomaticCodec(
    std::vector<std::unique_ptr<Codec>> customCodecs,
    std::unique_ptr<Codec> terminalCodec)
    : Codec(CodecType::USER_DEFINED, acc::none, "auto"),
...
#if (ACC_HAVE_LIBLZ4 && LZ4_VERSION_NUMBER >= 10301)
