...
std::unique_ptr<IOBuf> ZlibCodec::addOutputBuffer(z_stream* stream,
                                                  uint32_t length) {
  ACCCHECK_EQ(stream->avail_out, 0);
...
std::unique_ptr<IOBuf> ZlibCodec::doCompress(const IOBuf* data) {
  ...
  SCOPE_EXIT {
    ...
    // got dropped.
    ACCCHECK(rc == Z_OK || (!success && rc == Z_DATA_ERROR))
      << rc << ": " << stream.msg;
  };
  ...
  for (auto& range : *data) {
    ...
    while (remaining) {
      ...
      while (stream.avail_in != 0) {
        ...
        ACCCHECK_EQ(rc, Z_OK) << stream.msg;
      }
    }
  }
  ...
  ACCCHECK_EQ(rc, Z_STREAM_END) << stream.msg;
