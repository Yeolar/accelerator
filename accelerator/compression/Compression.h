...
#include "accelerator/compression/Counters.h"
...
class Codec {
 public:
   ...
   */
  std::unique_ptr<IOBuf> compress(const acc::IOBuf* data);
  ...
  std::unique_ptr<IOBuf> uncompress(
      const IOBuf* data,
      acc::Optional<uint64_t> uncompressedLength = acc::none);
   ...
   * is thrown.
   */
  acc::Optional<uint64_t> getUncompressedLength(
      const acc::IOBuf* data,
      acc::Optional<uint64_t> uncompressedLength = acc::none) const;

 protected:
  Codec(
      CodecType type,
      acc::Optional<int> level = acc::none,
 ...
 private:
  ...
  virtual std::unique_ptr<IOBuf> doCompress(const acc::IOBuf* data) = 0;
  virtual std::unique_ptr<IOBuf> doUncompress(
      const acc::IOBuf* data,
      acc::Optional<uint64_t> uncompressedLength) = 0;
  ...
  virtual std::string doUncompressString(
      StringPiece data,
      acc::Optional<uint64_t> uncompressedLength);

  virtual uint64_t doMaxCompressedLength(uint64_t uncompressedLength) const = 0;
  // default: returns the passed uncompressedLength.
  virtual acc::Optional<uint64_t> doGetUncompressedLength(
      const acc::IOBuf* data,
      acc::Optional<uint64_t> uncompressedLength) const;

  CodecType type_;
  acc::detail::CompressionCounter bytesBeforeCompression_;
  acc::detail::CompressionCounter bytesAfterCompression_;
  acc::detail::CompressionCounter bytesBeforeDecompression_;
  acc::detail::CompressionCounter bytesAfterDecompression_;
  acc::detail::CompressionCounter compressions_;
  acc::detail::CompressionCounter decompressions_;
  acc::detail::CompressionCounter compressionMilliseconds_;
  acc::detail::CompressionCounter decompressionMilliseconds_;
};
...
class StreamCodec : public Codec {
 public:
   ...
   * uncompressedLength.
   */
  void resetStream(acc::Optional<uint64_t> uncompressedLength = acc::none);
 ...
 private:
  ...
  std::unique_ptr<IOBuf> doCompress(const acc::IOBuf* data) override;
  std::unique_ptr<IOBuf> doUncompress(
      const acc::IOBuf* data,
      acc::Optional<uint64_t> uncompressedLength) override;
  ...
  acc::Optional<uint64_t> uncompressedLength_{};
