...
using not_ref_wrapper =
    acc::Negation<is_ref_wrapper<typename std::decay<T>::type>>;
...
struct return_type_helper<void, TList...> {
  static_assert(
      acc::Conjunction<not_ref_wrapper<TList>...>::value,
...
ACC_ALWAYS_INLINE ACC_ATTR_VISIBILITY_HIDDEN constexpr auto make_array_with(
    MakeItem const& make,
