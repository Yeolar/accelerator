...
#include "accelerator/MemoryMapping.h"
...
void MemoryMapping::init(off_t offset, off_t length) {
  ...
  ACCCHECK_GT(pageSize, 0);
  ACCCHECK_EQ(pageSize & (pageSize - 1), 0);  // power of two
  ACCCHECK_GE(offset, 0);
  ...
  if (mapLength_ == -1) {
    ...
    if (length > remaining) {
      if (grow) {
        if (!autoExtend) {
          ACCPCHECK(0 == ftruncate(file_.fd(), offset + length))
...
void MemoryMapping::advise(int advice, size_t offset, size_t length) const {
  ACCCHECK_LE(offset + length, size_t(mapLength_))
    << " offset: " << offset
    << " length: " << length
  ...
  ACCPLOG_IF(WARN, ::madvise(mapStart, length, advice)) << "madvise";
}
