      ...
      threads.emplace_back([&]() {
        ...
        while (reads + writes < itersPerThread) {
          ...
          if (reads < writes * readsPerWrite ||
            ...
            ++reads;
            auto b = ptr->get(key);
            acc::doNotOptimizeAway(b);
          } else {
            ++writes;
            try {
              ptr->update(key, range);
            } catch (std::bad_alloc&) {
              ACCLOG(INFO) << "bad alloc";
            }
          }
        }
      });
...
// sudo nice -n -20 ./accelerator/io/test/accelerator_io_FlatDictBenchmark -bm_min_iters 1000000
// ============================================================================
...
// sudo nice -n -20 ./accelerator/io/test/accelerator_io_FlatDictBenchmark
// ============================================================================
// FlatDictBenchmark.cpp                           relative  time/iter  iters/s
// ============================================================================
...
// ./accelerator/io/test/accelerator_io_FlatDictBenchmark -bm_min_iters 1 -bm_max_iters 2
// ============================================================================
// FlatDictBenchmark.cpp                           relative  time/iter  iters/s
// ============================================================================
