...
using acc::ByteRange;
using acc::IOBuf;
using acc::StringPiece;
using std::unique_ptr;
using namespace acc::io;
...
namespace {
void append(std::unique_ptr<IOBuf>& buf, acc::StringPiece data) {
...
TEST(IOBuf, Gather) {
  ...
  EXPECT_EQ("lo world",
            acc::StringPiece(reinterpret_cast<const char*>(cursor.data()),
                               cursor.length()));
...
TEST(IOBuf, cloneAndInsert) {
...

  {
    ...
    cursor.insert(acc::IOBuf::create(0));
