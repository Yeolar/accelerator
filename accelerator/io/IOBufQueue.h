...
class IOBufQueue {
 private:
  ...
  auto updateGuard() {
    flushCache();
    return acc::makeGuard([this] { updateWritableTailCache(); });
  }
 ...
 public:
  ...
  class WritableRangeCache {
   public:
    explicit WritableRangeCache(acc::IOBufQueue* q = nullptr) : queue_(q) {
      if (queue_) {
        fillCache();
      }
    }
    ...
    void append(size_t n) {
      ...
      // the length().
      if (ACC_LIKELY(data_.cachedRange.first != nullptr)) {
        DCHECK_LE(n, length());
   ...
   private:
    ...
    ACC_NOINLINE void appendSlow(size_t n) {
   ...
   */
  void append(std::unique_ptr<acc::IOBuf>&& buf, bool pack = false);
  ...
  std::pair<void*, std::size_t> preallocate(
    ...
    if (ACC_LIKELY(writableTail() != nullptr && tailroom() >= min)) {
   ...
   *         in the queue.
   */
  std::unique_ptr<acc::IOBuf> split(size_t n) {
    return split(n, true);
  }
   ...
   */
  std::unique_ptr<acc::IOBuf> splitAtMost(size_t n) {
   ...
   */
  std::unique_ptr<acc::IOBuf> move() {
    auto guard = updateGuard();
    std::unique_ptr<acc::IOBuf> res = std::move(head_);
    chainLength_ = 0;
    return res;
  }
   ...
   */
  std::unique_ptr<acc::IOBuf> pop_front();
  ...
  size_t chainLength() const {
    if (ACC_UNLIKELY(!options_.cacheChainLength)) {
 ...
 private:
  std::unique_ptr<acc::IOBuf> split(size_t n, bool throwOnUnderflow);
   ...
   */
  std::unique_ptr<acc::IOBuf> head_;
  ...
  void updateWritableTailCache() {
    if (ACC_LIKELY(head_ != nullptr)) {
      IOBuf* buf = head_->prev();
      if (ACC_LIKELY(!buf->isSharedOne())) {
