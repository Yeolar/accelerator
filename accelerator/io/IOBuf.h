...
class IOBuf {
 public:
  ...
  bool isSharedOne() const {
    ...
    if (ACC_UNLIKELY(!sharedInfo())) {
      return true;
    }

    if (ACC_UNLIKELY(sharedInfo()->externallyShared)) {
      return true;
    }

    if (ACC_LIKELY(!(flags() & kFlagMaybeShared))) {
      return false;
    }
  ...
  /**
   * Destructively convert this IOBuf to a std::string efficiently.
   */
