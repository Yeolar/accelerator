...
// namespace { acc::Singleton<MyExpensiveService> the_singleton; }
//
...
// struct Tag1 {};
// struct Tag2 {};
// acc::Singleton<MyExpensiveService> s_default;
// acc::Singleton<MyExpensiveService, Tag1> s1;
// acc::Singleton<MyExpensiveService, Tag2> s2;
// }
// ...
...
//
// namespace {
// acc::Singleton<MyExpensiveService> the_singleton(create, destroy);
// }
//
...
//   acc::Singleton<X> singleton_x([]() { return new X(42, "foo"); });
//
...
struct SingletonHolder {
 public:
  ...
  inline T* get() {
    if (ACC_LIKELY(state_.load(std::memory_order_acquire) == Living)) {
  ...
  inline std::weak_ptr<T> get_weak() {
    if (ACC_UNLIKELY(state_.load(std::memory_order_acquire) != Living)) {
      createInstance();
    }
