...
ThreadPoolExecutor::~ThreadPoolExecutor() {
  ACCCHECK_EQ(0, threadList_.get().size());
}
...
void ThreadPoolExecutor::runTask(const ThreadPtr& thread, Task&& task) {
  ...
  if (task.expiration_ > 0 && task.stats_.waitTime >= task.expiration_) {
    ...
    try {
      ...
      ACCLOG(ERROR) << "ThreadPoolExecutor: func threw unhandled "
                    << typeid(e).name() << " exception: " << e.what();
    } catch (...) {
      ACCLOG(ERROR) << "ThreadPoolExecutor: func threw unhandled non-exception "
                       "object";
    }
  ...
  *thread->taskStatsCallbacks->inCallback = true;
  ...
  try {
    ...
    ACCLOG(ERROR) << "ThreadPoolExecutor: task stats callback threw unhandled "
                  << typeid(e).name() << " exception: " << e.what();
  } catch (...) {
    ACCLOG(ERROR) << "ThreadPoolExecutor: task stats callback threw "
...
void ThreadPoolExecutor::setNumThreads(size_t n) {
  ...
  ACCCHECK_EQ(n, threadList_.get().size());
  ACCCHECK_EQ(0, stoppedThreads_.size());
}
...
void ThreadPoolExecutor::removeThreads(size_t n, bool isJoin) {
  ACCCHECK_LE(n, threadList_.get().size());
  isJoin_ = isJoin;
  stopThreads(n);
}
