...
#include "accelerator/Malloc.h"
#include "accelerator/Traits.h"

// When used in acc, assertions are not disabled.
...
ACC_DISABLE_WARNING("-Warray-bounds")

// FBString cannot use throw when replacing std::string, though it may still
// use std::__throw_*
// nolint
#define throw ACC_FBSTRING_MAY_NOT_USE_THROW

#define ACC_FBSTRING_BEGIN_NAMESPACE namespace acc {
#define ACC_FBSTRING_END_NAMESPACE } // namespace acc

ACC_FBSTRING_BEGIN_NAMESPACE
...
template <class Char> class fbstring_core {
 ...
 private:
  ...
  ACC_NOINLINE void destroyMediumLarge() noexcept {
...
template <class Char>
ACC_NOINLINE inline void fbstring_core<Char>::copyMedium(
...
template <class Char>
ACC_NOINLINE inline void fbstring_core<Char>::copyLarge(
    const fbstring_core& rhs) {
...
template <class Char>
ACC_NOINLINE inline void fbstring_core<Char>::initMedium(
    const Char* const data, const size_t size) {
...
template <class Char>
ACC_NOINLINE inline void fbstring_core<Char>::initLarge(
    const Char* const data, const size_t size) {
...
template <class Char>
ACC_NOINLINE inline void fbstring_core<Char>::unshare(
    size_t minCapacity) {
...
template <class Char>
ACC_NOINLINE inline void fbstring_core<Char>::reserveLarge(
    size_t minCapacity) {
  FBSTRING_ASSERT(category() == Category::isLarge);
  if (RefCounted::refs(ml_.data_) > 1) { // Ensure unique
...
template <class Char>
ACC_NOINLINE inline void fbstring_core<Char>::reserveMedium(
...
template <class Char>
ACC_NOINLINE inline void fbstring_core<Char>::reserveSmall(
 ...
 public:
  ...
  typename std::enable_if<
      std::is_same<
          ...
          typename acc::basic_fbstring<E, T, A, Storage>::value_type>::value,
...
ACC_NOINLINE inline typename basic_fbstring<E, T, A, S>::size_type
...
template <typename E, class T, class A, class S>
ACC_NOINLINE inline basic_fbstring<E, T, A, S>&
...
ACC_ASSUME_RELOCATABLE(basic_fbstring<T, R, A, S>);

ACC_FBSTRING_END_NAMESPACE
...
#define ACC_FBSTRING_HASH1(T)                                        \
  template <>                                                        \
  struct hash< ::acc::basic_fbstring<T>> {                           \
    size_t operator()(const ::acc::basic_fbstring<T>& s) const {     \
      return ::acc::hash::fnv32_buf(s.data(), s.size() * sizeof(T)); \
    }                                                                \
  };

// The C++11 standard says that these four are defined
#define ACC_FBSTRING_HASH \
  ACC_FBSTRING_HASH1(char) \
  ACC_FBSTRING_HASH1(char16_t) \
  ACC_FBSTRING_HASH1(char32_t) \
  ACC_FBSTRING_HASH1(wchar_t)
...
#undef ACC_FBSTRING_HASH
#undef ACC_FBSTRING_HASH1
