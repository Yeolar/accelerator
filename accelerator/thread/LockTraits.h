 ...
 * acc::Synchronized to support a number of different mutex types.
 */
#pragma once
...
// OSX don't have timed mutexes
#if defined(__APPLE__)
#define ACC_LOCK_TRAITS_HAVE_TIMED_MUTEXES 0
#else
#define ACC_LOCK_TRAITS_HAVE_TIMED_MUTEXES 1
#endif
...
namespace member {
ACC_CREATE_MEMBER_INVOKE_TRAITS(lock, lock);
ACC_CREATE_MEMBER_INVOKE_TRAITS(try_lock_for, try_lock_for);
ACC_CREATE_MEMBER_INVOKE_TRAITS(lock_shared, lock_shared);
ACC_CREATE_MEMBER_INVOKE_TRAITS(lock_upgrade, lock_upgrade);
} // namespace member
