...
#include "accelerator/thread/ConcurrentSkipList-inl.h"
...
class ConcurrentSkipList {
  ...
  typedef std::unique_lock<acc::MicroSpinLock> ScopedLocker;
  ...
  std::pair<NodeType*, size_t> addOrGetData(U &&data) {
    ...
    while (true) {
      ...
      if (layer >= 0) {
        ...
        while (ACC_UNLIKELY(!nodeFound->fullyLinked())) {}
