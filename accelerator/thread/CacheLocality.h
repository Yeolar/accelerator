...
#define ACC_ALIGN_TO_AVOID_FALSE_SHARING ACC_ALIGNED(128)
...
/// A class that lazily binds a unique identifier to a thread.
/// This is a fallback mechanism for the access spreader if __vdso_getcpu
/// can't be loaded
struct FallbackGetcpu {
 ...
 private:
  static std::atomic<unsigned> prevId;
...
/// loaded.  Using sched_getcpu or the getcpu syscall would negate the
/// performance advantages of access spreading, so we use a thread-local
/// value and a shared atomic counter to spread access out.
struct AccessSpreader {
 ...
 private:
  ...
  /// Returns the best getcpu implementation
  static Getcpu::Func pickGetcpuFunc() {
    auto best = Getcpu::resolveVdsoFunc();
    return best ? best : &FallbackGetcpu::getcpu;
  }
  ...
  static bool initialize() {
    ...
    auto& cacheLocality = CacheLocality::system();
