...
class MPMCQueue {
  static_assert(std::is_nothrow_constructible<T,T&&>::value ||
                acc::IsRelocatable<T>::value,
      "T must be relocatable or have a noexcept move constructor");

 public:
  typedef T value_type;

  using Slot = detail::SingleElementQueue<T>;
  ...
  void blockingRead(T& elem) noexcept {
    assert(capacity_ != 0);
    dequeueWithTicketBase(popTicket_++, slots_, capacity_, stride_, elem);
  }
 ...
 protected:
  ...
  void enqueueWithTicketBase(
    ...
    slots[idx(ticket, cap, stride)].enqueue(turn(ticket, cap),
                                            pushSpinCutoff_,
  ...
  void dequeueWithTicketBase(
    ...
    assert(cap != 0);
    slots[idx(ticket, cap, stride)].dequeue(turn(ticket, cap),
                                            popSpinCutoff_,
                                            (ticket % kAdaptationFreq) == 0,
                                            elem);
  }
...
struct SingleElementQueue {
  ...
  void dequeue(uint32_t turn,
    ...
    dequeueImpl(turn,
                ...
                updateSpinCutoff,
                elem,
                typename std::conditional<acc::IsRelocatable<T>::value,
                                          ImplByRelocation,
                                          ImplByMove>::type());
  }
