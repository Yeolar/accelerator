 ...
 */
#include "accelerator/thread/detail/ThreadLocalDetail.h"
...
namespace acc { namespace threadlocal_detail {

void ThreadEntryNode::initIfZero(bool locked) {
  if (ACC_UNLIKELY(!next)) {
    if (ACC_LIKELY(locked)) {
...
ThreadEntryList* StaticMetaBase::getThreadEntryList() {
  ...
  class PthreadKey {
   public:
    ...
    ACC_ALWAYS_INLINE pthread_key_t get() const {
      return pthreadKey_;
    }
  ...
  if (ACC_UNLIKELY(!threadEntryList)) {
...
uint32_t StaticMetaBase::elementsCapacity() const {
  ...
  return ACC_LIKELY(!!threadEntry) ? threadEntry->getElementsCapacity() : 0;
}
...
void StaticMetaBase::destroy(EntryID* ent) {
  try {
    ...
    ACCLOG(WARNING) << "Destructor discarding an exception that was thrown.";
  }
}
...
ElementWrapper* StaticMetaBase::reallocate(
  ...
  // Growth factor < 2, see acc/docs/FBVector.md; + 5 to prevent
  // very slow start.
