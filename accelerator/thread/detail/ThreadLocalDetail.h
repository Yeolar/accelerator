...
#include "accelerator/detail/StaticSingletonManager.h"
...
#define ACC_TLD_USE_ACC_TLS 1
#else
#undef ACC_TLD_USE_ACC_TLS
#endif
...
struct ThreadEntryNode {
  ...
  ACC_ALWAYS_INLINE bool empty() const {
    return (next == parent);
  }

  ACC_ALWAYS_INLINE bool zero() const {
    return (!prev);
  }

  ACC_ALWAYS_INLINE ThreadEntry* getThreadEntry() {
    return parent;
  }

  ACC_ALWAYS_INLINE ThreadEntryNode* getPrev();

  ACC_ALWAYS_INLINE ThreadEntryNode* getNext();
...
ACC_ALWAYS_INLINE ThreadEntryNode* ThreadEntryNode::getPrev() {
  return &prev->elements[id].node;
}

ACC_ALWAYS_INLINE ThreadEntryNode* ThreadEntryNode::getNext() {
...
class PthreadKeyUnregister {
 ...
 private:
  ...
  friend struct acc::threadlocal_detail::PthreadKeyUnregisterTester;
...
struct StaticMetaBase {
  ...
  StaticMetaBase(ThreadEntry* (*threadEntry)());
  ...
  ACC_EXPORT static ThreadEntryList* getThreadEntryList();
  ...
  std::mutex lock_;
  boost::shared_mutex accessAllThreadsLock_;
...
struct StaticMeta : StaticMetaBase {
  ...
  static StaticMeta<Tag>& instance() {
    ...
    /* library-local */ static auto instance =
        detail::createGlobal<StaticMeta<Tag>, void>();
    return *instance;
  }
  ...
  static void getSlowReserveAndCache(
    ...
    if (ACC_UNLIKELY(threadEntry->getElementsCapacity() <= id)) {
      inst.reserve(ent);
      id = ent->getOrInvalid();
    }
  ...
  ACC_EXPORT ACC_NOINLINE static ThreadEntry* getThreadEntrySlow() {
    auto& meta = instance();
