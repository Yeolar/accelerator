...
#include "accelerator/thread/CacheLocality.h"
...
//
// SharedMutex provides all of the methods of acc::RWSpinLock,
...
// I compared it to acc::RWSpinLock, acc::RWTicketSpinLock64,
...
// of acc/test/SharedMutexTest.cpp for lots of microbenchmark results.
//
// Comparison to acc::RWSpinLock:
//
...
class SharedMutexImpl {
 ...
 private:
  typedef typename acc::Futex Futex;
  ...
  struct WaitForDuration {
    ...
    bool doWait(Futex& futex, uint32_t expected, uint32_t waitMask) {
      ...
      return result != acc::FutexResult::TIMEDOUT;
    }
  };
  ...
  static constexpr uint32_t kMaxSpinCount = 1000;
  ...
  static constexpr uint32_t kMaxSoftYieldCount = 1000;
 ...
 private:
  ...
  // See kTokenless.
 public:
  typedef std::atomic<uintptr_t> DeferredReaderSlot;

 private:
  ACC_ALIGN_TO_AVOID_FALSE_SHARING static DeferredReaderSlot deferredReaders
      [kMaxDeferredReaders *
  ...
  // an array of std::atomic<uintptr_t>.
...
template <bool ReaderPriority, typename Tag_>
typename SharedMutexImpl<ReaderPriority, Tag_>::DeferredReaderSlot
SharedMutexImpl<ReaderPriority, Tag_>::
    deferredReaders[kMaxDeferredReaders * kDeferredSeparationFactor] = {};

template <bool ReaderPriority, typename Tag_>
__thread uint32_t
SharedMutexImpl<ReaderPriority, Tag_>::tls_lastTokenlessSlot = 0;

template <bool ReaderPriority, typename Tag_>
__thread uint32_t
SharedMutexImpl<ReaderPriority, Tag_>::tls_lastDeferredReaderSlot = 0;

template <bool ReaderPriority, typename Tag_>
bool SharedMutexImpl<ReaderPriority, Tag_>::
    tryUnlockTokenlessSharedDeferred() {
  ...
  while (true) {
    ...
    if (canAlreadyDefer || (aboveDeferThreshold && !drainInProgress)) {
      ...
      if (slotValue != 0) {
        ...
        uint32_t bestSlot =
            (uint32_t)acc::AccessSpreader::current(kMaxDeferredReaders);
