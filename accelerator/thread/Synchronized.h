 ...
 * module.  See acc/docs/Synchronized.md for a more complete explanation of
...
#include "accelerator/Foreach.h"
#include "accelerator/Logging.h"
#include "accelerator/Utility.h"
#include "accelerator/thread/LockTraits.h"
...
class SynchronizedBase<Subclass, detail::MutexLevel::SHARED> {
 public:
  using LockedPtr = ::acc::LockedPtr<Subclass, LockPolicyExclusive>;
  using ConstWLockedPtr =
      ::acc::LockedPtr<const Subclass, LockPolicyExclusive>;
  using ConstLockedPtr = ::acc::LockedPtr<const Subclass, LockPolicyShared>;

  using TryWLockedPtr = ::acc::LockedPtr<Subclass, LockPolicyTryExclusive>;
  using ConstTryWLockedPtr =
      ::acc::LockedPtr<const Subclass, LockPolicyTryExclusive>;
  using TryRLockedPtr = ::acc::LockedPtr<const Subclass, LockPolicyTryShared>;
...
class SynchronizedBase<Subclass, detail::MutexLevel::UPGRADE>
 ...
 public:
  using UpgradeLockedPtr = ::acc::LockedPtr<Subclass, LockPolicyUpgrade>;
  using ConstUpgradeLockedPtr =
      ::acc::LockedPtr<const Subclass, LockPolicyUpgrade>;

  using TryUpgradeLockedPtr =
      ::acc::LockedPtr<Subclass, LockPolicyTryUpgrade>;
  using ConstTryUpgradeLockedPtr =
      ::acc::LockedPtr<const Subclass, LockPolicyTryUpgrade>;
...
class SynchronizedBase<Subclass, detail::MutexLevel::UNIQUE> {
 public:
  ...
  using TryLockedPtr = ::acc::LockedPtr<Subclass, LockPolicyTryExclusive>;
 ...
 * acc::RWSpinLock, and acc::SpinLock.
 ...
 */
template <class T, class Mutex = /*SharedMutex*/ std::mutex>
struct Synchronized : public SynchronizedBase<
 ...
 private:
  ...
  friend class acc::LockedPtrBase;
  template <class LockedType, class LockPolicy>
  friend class acc::LockedPtr;
 ...
 * See the benchmarks in acc/test/SynchronizedBenchmark.cpp for more.
 */
...
class LockedPtrBase {
 public:
  using MutexType = Mutex;
  friend class acc::ScopedUnlocker<SynchronizedType, LockPolicy>;
 ...
 * using one of the four strategies - acc::lock (exclusive acquire for
 * exclusive only mutexes), acc::rlock (shared acquire for shareable
 * mutexes), acc::wlock (exclusive acquire for shareable mutexes) or
 * acc::ulock (upgrade acquire for upgrade mutexes) (see above)
 *
 ...
 * other threads.  See the benchmarks in acc/test/SynchronizedBenchmark.cpp
 * for more
 *
 ...
 *
 *    acc::lock(one, two, three);
 *
 * To make it work with acc::Synchronized you have to specify how you want
 * the locks to be acquired, use the acc::wlock(), acc::rlock(),
 * acc::ulock() and acc::lock() helpers defined below
 *
 *    auto [one, two] = lock(acc::wlock(a), acc::rlock(b));
 *
 * Note that you can/must avoid the acc:: namespace prefix on the lock()
 ...
 *
 * use lock() with acc::wlock(), acc::rlock() and acc::ulock() for
 ...
 */
#define SYNCHRONIZED_VAR(var) ACC_CONCATENATE(SYNCHRONIZED_##var##_, __LINE__)
 ...
 *
 * Refer to acc/docs/Synchronized.md for a detailed explanation and more
 * examples.
 */
#define SYNCHRONIZED(...)                                             \
  ACC_PUSH_WARNING                                                    \
  ACC_GNU_DISABLE_WARNING("-Wshadow")                                 \
  ACC_MSVC_DISABLE_WARNING(4189) /* initialized but unreferenced */   \
  ACC_MSVC_DISABLE_WARNING(4456) /* declaration hides local */        \
  ACC_MSVC_DISABLE_WARNING(4457) /* declaration hides parameter */    \
  ACC_MSVC_DISABLE_WARNING(4458) /* declaration hides member */       \
  ACC_MSVC_DISABLE_WARNING(4459) /* declaration hides global */       \
  ACC_GCC_DISABLE_NEW_SHADOW_WARNINGS                                 \
  if (bool SYNCHRONIZED_VAR(state) = false) {                         \
  } else                                                              \
    for (auto SYNCHRONIZED_VAR(lockedPtr) =                           \
             (ACC_VA_GLUE(ACC_ARG_2_OR_1, (__VA_ARGS__))).operator->(); \
         !SYNCHRONIZED_VAR(state);                                    \
         SYNCHRONIZED_VAR(state) = true)                              \
      for (auto& ACC_VA_GLUE(ACC_ARG_1, (__VA_ARGS__)) =              \
...
#define TIMED_SYNCHRONIZED(timeout, ...)                                       \
  if (bool SYNCHRONIZED_VAR(state) = false) {                                  \
    ...
    for (auto SYNCHRONIZED_VAR(lockedPtr) =                                    \
             (ACC_VA_GLUE(ACC_ARG_2_OR_1, (__VA_ARGS__))).timedAcquire(timeout); \
         !SYNCHRONIZED_VAR(state);                                             \
         SYNCHRONIZED_VAR(state) = true)                                       \
      for (auto ACC_VA_GLUE(ACC_ARG_1, (__VA_ARGS__)) =                        \
...
#define SYNCHRONIZED_CONST(...)            \
  SYNCHRONIZED(                            \
      ACC_VA_GLUE(ACC_ARG_1, (__VA_ARGS__)), \
      as_const(ACC_VA_GLUE(ACC_ARG_2_OR_1, (__VA_ARGS__))))
