...
class SynchronizedBase<Subclass, detail::MutexLevel::UPGRADE>
 ...
 public:
  ...
  using UpgradeLockedGuardPtr =
      ::acc::LockedGuardPtr<Subclass, LockPolicyUpgrade>;
  using ConstUpgradeLockedGuardPtr =
      ::acc::LockedGuardPtr<const Subclass, LockPolicyUpgrade>;
 ...
 * acc::SharedMutex, acc::RWSpinLock, and acc::SpinLock.
...
struct Synchronized : public SynchronizedBase<
 ...
 private:
  ...
  template <class LockedType, class LockPolicy>
  friend class acc::LockedGuardPtr;
...
#define SYNCHRONIZED(...)                                        \
  if (bool SYNCHRONIZED_VAR(state) = false) {                    \
    ...
    for (auto SYNCHRONIZED_VAR(lockedPtr) =                      \
           (ACC_ARG_2_OR_1(__VA_ARGS__)).operator->();           \
         !SYNCHRONIZED_VAR(state);                               \
         SYNCHRONIZED_VAR(state) = true)                         \
      for (auto& ACC_ARG_1(__VA_ARGS__) =                        \
...
#define TIMED_SYNCHRONIZED(timeout, ...)                         \
  if (bool SYNCHRONIZED_VAR(state) = false) {                    \
    ...
    for (auto SYNCHRONIZED_VAR(lockedPtr) =                      \
           (ACC_ARG_2_OR_1(__VA_ARGS__)).timedAcquire(timeout);  \
         !SYNCHRONIZED_VAR(state);                               \
         SYNCHRONIZED_VAR(state) = true)                         \
      for (auto ACC_ARG_1(__VA_ARGS__) =                         \
...
#define SYNCHRONIZED_CONST(...)                \
  SYNCHRONIZED(                                \
      ACC_ARG_1(__VA_ARGS__),                  \
      (ACC_ARG_2_OR_1(__VA_ARGS__)).asConst())
