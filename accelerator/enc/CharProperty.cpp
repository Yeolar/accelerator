...
bool CharProperty::open(const std::string& filename) {
  if (!readFile(filename.c_str(), data_)) {
    ACCLOG(ERROR) << "read char property file " << filename << " failed";
    return false;
  }
  ...
  if (data_.size() != (offset + kMaxCategoryLength * n
    ...
    ACCLOG(ERROR) << "illegal size of char property file " << filename;
    return false;
  }
  categories_.clear();
...
CharRange parseCharRangeFromLine(StringPiece line) {
  ...
  if (range.l >= 0xffff || range.h >= 0xffff || range.l > range.h) {
    ACCLOG(ERROR) << "code point range error: " << cols[0];
    return range;
  }
...
CharInfo encode(const std::vector<StringPiece>& c,
  ...
  CharInfo base;
  if (c.empty()) {
    ACCLOG(ERROR) << "no category";
    return base;
  }
  for (auto& i : c) {
    auto it = m.find(i);
    if (it == m.end()) {
      ACCLOG(ERROR) << "category[" << i << "] is undefined";
      return base;
    }
    base += it->second;
  }
...
bool CharProperty::compile(const std::string& cfile,
  ...
  if (!readFile(cfile.c_str(), conf)) {
    ACCLOG(ERROR) << "read conf file " << cfile << " failed";
    return false;
  }
  ...
  for (auto& line : lines) {
    ...
    if (line.subpiece(0, 2) != "0x") {
      ...
      if (c.size() >= kMaxCategoryLength) {
        ACCLOG(ERROR) << "category[" << c << "] too long";
        return false;
      }
      if (cmap.find(c) != cmap.end()) {
        ACCLOG(ERROR) << "category[" << c << "] is already defined";
        return false;
      }
  ...
  if (charset == "gbk") {
    ...
    ACCLOG(ERROR) << "illegal encoding: " << charset;
    return false;
  }
