...
/*
 * Defines a function acc::applyTuple, which takes a function and a
 * std::tuple of arguments and calls the function with those
 * arguments.
 *
 * Example:
 *
 *    int x = acc::applyTuple(std::plus<int>(), std::make_tuple(12, 12));
 *    ASSERT(x == 24);
 */
...
template <typename... Tuples>
using MakeIndexSequenceFromTuple = acc::make_index_sequence<
...
inline constexpr auto call(F&& f, Tuple&& t, acc::index_sequence<Indexes...>)
    -> decltype(
...
inline constexpr auto forwardTuple(Tuple&& t, acc::index_sequence<Indexes...>)
    -> decltype(
